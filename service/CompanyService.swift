...
class ApiCompanyService {
    ...
    static func uploadImageToServer(image: UIImage,parameters:[String:Any]) {
       ...
       guard let url = URL(string: "https://backend-showapp.herokuapp.com/company/signup") else { return }
       var request = URLRequest(url: url)
       request.httpMethod = "POST"
       //create boundary
    ...
    static func addNewsToServer(image: UIImage,parameters:[String:Any]) {
           ...
           guard let url = URL(string: "https://backend-showapp.herokuapp.com/news") else { return }
           var request = URLRequest(url: url)
           request.httpMethod = "POST"
           //create boundary
           let boundary = generateBoundary()
    ...
    func getCompanyNews( callback: @escaping (Bool,News?)->Void){
        ...
        guard let url = URL(string: "https://backend-showapp.herokuapp.com/news/brand/" + UserDefaults.standard.string(forKey: "_id")!) else{
                    return
                }
                var request = URLRequest(url: url)
    ...
    func getAllCompanyNews( callback: @escaping (Bool,News?)->Void){
               
        guard let url = URL(string: "https://backend-showapp.herokuapp.com/news") else{
                    return
                }
                var request = URLRequest(url: url)
                request.httpMethod = "GET"
    ...
    func getAllBrands( callback: @escaping (Bool,Companys?)->Void){
               
        guard let url = URL(string: "https://backend-showapp.herokuapp.com/company") else{
                    return
                }
                var request = URLRequest(url: url)
                request.httpMethod = "GET"
                ...
                let session = URLSession.shared.dataTask(with: request){
                    ...
                    if error == nil && data != nil{
                        ...
                        do {
                            ...
                            let test = try decoder.decode(Companys.self, from: data!)
                            callback(true,test)
                        } catch  {
                            print(error)
                            callback(false,nil)
