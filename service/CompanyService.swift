...
class ApiCompanyService {
    ...
    static func uploadImageToServer(image: UIImage,parameters:[String:Any]) {
       ...
       guard let url = URL(string: "http://192.168.1.12:3000/company/signup") else { return }
       var request = URLRequest(url: url)
       request.httpMethod = "POST"
       //create boundary
    ...
    static func addNewsToServer(image: UIImage,parameters:[String:Any]) {
           ...
           guard let url = URL(string: "http://192.168.1.12:3000/news") else { return }
           var request = URLRequest(url: url)
           request.httpMethod = "POST"
           //create boundary
           let boundary = generateBoundary()
    ...
    func getCompanyNews( callback: @escaping (Bool,News?)->Void){
        ...
        guard let url = URL(string: "http://192.168.1.12:3000/news/brand/" + UserDefaults.standard.string(forKey: "_id")!) else{
                    return
                }
                var request = URLRequest(url: url)
    ...
    func getAllCompanyNews( callback: @escaping (Bool,News?)->Void){
        ...
        guard let url = URL(string: "http://192.168.1.12:3000/news") else{
                    return
                }
                var request = URLRequest(url: url)
                request.httpMethod = "GET"
    ...
    func getAllBrands( callback: @escaping (Bool,Companys?)->Void){
        ...
        guard let url = URL(string: "http://192.168.1.12:3000/company") else{
                    return
                }
                var request = URLRequest(url: url)
                request.httpMethod = "GET"
