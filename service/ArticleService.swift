...
class articleService {
    ...
    func getBrandArticle(BrandId:String, callback: @escaping (Bool,Articles?)->Void){
            ...
            guard let url = URL(string: "http://192.168.1.13:3000/articles/company/"+BrandId ) else{
                return
            }
            var request = URLRequest(url: url)
            request.httpMethod = "GET"
    ...
    func getUserFavourites( callback: @escaping (Bool,Favorites?)->Void){
        ...
        guard let url = URL(string: "http://192.168.1.13:3000/favorites/refuser/"+UserDefaults.standard.string(forKey: "_id")!) else{
                    return
                }
                var request = URLRequest(url: url)
                request.httpMethod = "GET"
    ...
    func getUserCarts( callback: @escaping (Bool,Factures?)->Void){
        ...
        guard let url = URL(string: "http://192.168.1.13:3000/factures/refuser/"+UserDefaults.standard.string(forKey: "_id")!) else{
                    return
                }
                var request = URLRequest(url: url)
                request.httpMethod = "GET"
