...
//        }
    ...
    @IBAction func addToFavourite(_ sender: UIButton) {
                if isActive {
                    isActive = false
                    guard let url = URL(string: "http://192.168.1.12:3000/favorites/"+article!._id+"/"+UserDefaults.standard.string(forKey: "_id")!) else {
        ...
        sender.setImage(UIImage(named: "whiteHeart"), for: .normal)
                ...
                guard let url = URL(string: "http://192.168.1.12:3000/favorites/add") else {
                            fatalError("Error getting the url")
                        }
