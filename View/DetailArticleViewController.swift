...
//        }
    ...
    @IBAction func addToFavourite(_ sender: UIButton) {
                if isActive {
                    isActive = false
                    guard let url = URL(string: "http://172.31.32.1:3000/favorites/"+article!._id+"/"+UserDefaults.standard.string(forKey: "_id")!) else {
        ...
        sender.setImage(UIImage(named: "whiteHeart"), for: .normal)
                ...
                guard let url = URL(string: "http://172.31.32.1:3000/favorites/add") else {
                            fatalError("Error getting the url")
                        }
    ...
    @IBAction func addToCart(_ sender: Any) {
        ...
        guard let url = URL(string: "http://172.31.32.1:3000/factures/add") else {
                    fatalError("Error getting the url")
                }
